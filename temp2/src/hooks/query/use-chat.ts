...
export function useGetChatById(id: string, projectId: string) {
    const { data, error, isLoading, mutate } = useSWR(`/api/project/${projectId}/chat?id=${id}`, fetcher);
    return {
        data: data || [],
...
export function useGetMessagesByChatId(chatId: string, projectId: string) {
    const { data, error, isLoading, mutate } = useSWR(
        `/api/project/${projectId}/chat/messages?chatId=${chatId}`,
        fetcher
    );
    return {
        data: data || [],
...
export function getChatHistoryPaginationKey(
    ...
    // 第一页
    if (pageIndex === 0) {
        return `/api/project/${projectId}/chat/history?limit=${PAGE_SIZE}`;
    }
    ...
    return `/api/project/${projectId}/chat/history?ending_before=${lastChat.id}&limit=${PAGE_SIZE}`;
}
