...
import type {QuestionsWithDatasetSample,DatasetSamples} from '@/types/interfaces';
...
export function DataTable<TData extends QuestionsWithDatasetSample, TValue>({
                                                                                columns,
                                                                                data,
                                                                                pageCount,
                                                                                pagination,
    ...
    const handlePrimaryAnswer = (datasetSample: DatasetSamples) => {
        apiClient.patch(`/${datasetSample.projectId}/qa-dataset/primary-answer/${datasetSample.id}`)
            .then(_ => {
                toast.success('设置成功');
                refresh();
            })
    ...
    const deleteDataset = async (dataset: DatasetSamples, row: any) => {
        const res = await apiClient.delete(`/${dataset.projectId}/qa-dataset/delete?ids=${dataset.id}`);
        if (res.status === 200) {
            toast.success('删除成功');
            void refresh();
                                                    ...
                                                    <div className="space-y-2">
                                                        {row.original.DatasetSamples.sort((a, b) => {
                                                            ...
                                                            >
                                                                ...
                                                                {!dataset.isPrimaryAnswer && (
                                                                    <Separator
                                                                        className="mt-3 group-last:hidden opacity-50"/>
                                                                )}
                                                            </div>
                                                        ))}
                                                    </div>
