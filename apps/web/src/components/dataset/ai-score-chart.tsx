...
export const AIScoreDashboard = ({dssId, contextType}: { dssId: string; contextType: UIContextType }) => {
    ...
    const handleAIScore = () => {
        ...
        toast.promise(apiClient.post(`/${projectId}/qa-dataset/ai-score`, {dssId: dssId, modelId: model.id}), {
                loading: 'AI 正在进行评分中...',
                success: _ => {
                    void refresh();
                    return '处理成功';
        ...
        <>
            {datasetEvalList.length > 0 ? (
                <Tabs defaultValue={datasetEvalList[0]?.id || 'new'} className="items-center">
                    <TabsList
                        className="text-foreground h-auto gap-2 rounded-none border-b bg-transparent px-0 py-1 overflow-x-auto scrollbar-hide">
                ...
                <div className="text-center py-6">
                    <div
                        className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-indigo-500 mx-auto"></div>
