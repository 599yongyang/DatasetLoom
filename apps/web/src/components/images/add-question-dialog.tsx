...
export default function AddQuestionDialog({
    ...
    // 准备提及数据
    const mentionData = image.ImageBlock.map(block => ({
        id: block.id,
        display: block.label
    }));
    ...
    const handleGenerateAIQuestions = async () => {
        if (!image.ImageBlock || !image.ImageBlock.length || !customPrompt.trim()) {
        ...
        apiClient.post(`/${projectId}/image-chunk/gen-question`, {
            imageId: image.id,
            prompt: customPrompt,
            modelId: modelValue
        })
