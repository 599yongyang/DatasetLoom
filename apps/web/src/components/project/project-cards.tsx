...
export function ProjectCards({ projects, getProjects }: { projects: ProjectsWithCounts[]; getProjects: () => void }) {
    ...
    const deleteProject = (id: string) => {
        toast.promise(apiClient.delete(`/project/${id}`), {
            success: () => {
                getProjects();
    ...
    return (
        <div
            className="*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:px-20 @xl/main:grid-cols-2 @5xl/main:grid-cols-3">
