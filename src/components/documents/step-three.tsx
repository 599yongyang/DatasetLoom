...
export default function StepThree({ uploadFormData }: { uploadFormData: UploadFormDataType }) {
    ...
    const handleProcess = async () => {
        try {
            ...
            //解析文档
            const parseRes = await axios.post(`/api/project/${projectId}/documents/parser`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });

            if (!parseRes.data.success) throw new Error('文档解析失败');

            // 开始分块
            setStatus('chunking');

            const chunkRes = await axios.post(`/api/project/${projectId}/documents/chunker`, {
                ...uploadFormData,
                fileIds: parseRes.data.data
            });
            if (!chunkRes.data.success) throw new Error('文档分块失败');

            // 成功处理
            setStatus('done');
            const { chunkList, hash } = chunkRes.data;
