...
export function ProviderDialog({
    ...
    const handleSubmit = () => {
        const check = DEFAULT_PROVIDERS.find(item => item.id === provider.name.toLowerCase());
        if (check) {
            toast.warning('此服务商已存在');
            return;
        }
        if (!provider.name) {
            toast.error('请填写模型名称');
        ...
        axios
            .post(`/api/project/${projectId}/providers`, {
                provider: { ...provider, id: nanoid() },
                type: 'add'
            })
            .then(res => {
                toast.success('保存成功');
            ...
            <AlertDialogContent className="flex flex-col gap-0 overflow-y-visible p-0 sm:max-w-lg [&>button:last-child]:top-3.5">
                ...
                <div className="overflow-y-auto">
                    <div className="px-6 flex flex-1 items-center justify-between">
                        <div className="border-background bg-muted relative flex size-18 items-center justify-center overflow-hidden rounded-full border-4 shadow-xs shadow-black/10">
